...
$qry = $conn->query("SELECT r.*,c.category,c.balance from `running_balance` r inner join `categories` c on r.category_id = c.id where c.status= 1 and r.balance_type = 2 and r.user_id = '{$user_id}' order by unix_timestamp(r.date_created) desc");
